...
final class UIRenderer {
    ...
    void fillEmojiPanel(View panelView, EmojiEngine engine,
        ...
        int radius = ime.dp(ime.getKeyRadiusDp());
        int strokeW = ime.dp(ime.getKeyStrokeWidthDp());
        int strokeC = ime.getKeyStrokeColor();
        ...
        for (int i = 0; i < EmojiData.CATEGORY_KEYS.length; i++) {
            ...
            tab.setBackground(roundedFill(active ? CodeKeysIME.blend(keyBg, accent, 0.30f) : keyBg, radius, strokeW, strokeC));
        ...
        // We avoid removeAllViews() for the whole grid to reduce layout churn.
        // Instead, we'll sync the rows and their children.
            ...
            final GestureDetector swipe = new GestureDetector(ime,
                    new GestureDetector.SimpleOnGestureListener() {
                        ...
                        public boolean onFling(MotionEvent e1, MotionEvent e2,
                            ...
                            int idx = -1;
                            for (int j = 0; j < EmojiData.CATEGORY_KEYS.length; j++) {
                                if (EmojiData.CATEGORY_KEYS[j].equals(engine.getCategory())) {
                                    idx = j; break;
                                }
                            }
        ...
        final int columns = 8;
        int rowCount = (emojis.size() + columns - 1) / columns;

        // Synchronize row count
        while (grid.getChildCount() > rowCount) {
        ...
        for (int r = 0; r < rowCount; r++) {
            ...
            for (int c = 0; c < columns; c++) {
                ...
                if (index < emojis.size()) {
                    final String emoji = emojis.get(index);
                    btn.setVisibility(View.VISIBLE);
                    btn.setText(emoji);
                    btn.setBackground(roundedFill(keyBg, radius, strokeW, strokeC));
    ...
    private View buildClipCard(final ClipboardStore.Entry entry,
        ...
        card.setPadding(ime.dp(12), ime.dp(8), ime.dp(8), ime.dp(8));
        ...
        cardLp.setMargins(ime.dp(6), ime.dp(4), ime.dp(6), ime.dp(4));
        card.setLayoutParams(cardLp);

        // Text preview (max 2 lines, ellipsis)
        TextView preview = new TextView(ime);
        preview.setText(entry.text);
        ...
        // Pin toggle (ImageButton, rounded background)
        ImageButton pin = new ImageButton(ime);
        pin.setImageResource(R.drawable.pin);
        pin.setScaleType(ImageView.ScaleType.FIT_CENTER);
        pin.setPadding(ime.dp(10), ime.dp(10), ime.dp(10), ime.dp(10));
        pin.setColorFilter(entry.pinned ? accent : dim(textCol));
        pin.setBackground(roundedFill(CodeKeysIME.blend(keyBg, accent, 0.10f), radius, strokeW, strokeC));
        LinearLayout.LayoutParams pinLp = new LinearLayout.LayoutParams(ime.dp(38), ime.dp(38));
        pinLp.setMargins(ime.dp(4), 0, ime.dp(2), 0);
        pin.setLayoutParams(pinLp);
        pin.setOnClickListener(v -> onPin.onEntry(entry));
        card.addView(pin);

        // Delete (ImageButton, error-tinted)
        ImageButton del = new ImageButton(ime);
        del.setImageResource(R.drawable.x);
        del.setScaleType(ImageView.ScaleType.FIT_CENTER);
        del.setPadding(ime.dp(10), ime.dp(10), ime.dp(10), ime.dp(10));
        del.setColorFilter(0xFFFF6666);
        del.setBackground(roundedFill(0x22FF0000, radius, strokeW, strokeC));
        LinearLayout.LayoutParams dLp = new LinearLayout.LayoutParams(ime.dp(38), ime.dp(38));
